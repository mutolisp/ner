...
Public Const dc_water As Double = 322#        'critical density in kg/m**3
Private ireg1(1 To 34) As Integer
Private jreg1(1 To 34) As Integer
Private nreg1(1 To 34) As Double
Private j0reg2(1 To 9) As Integer
Private n0reg2(1 To 9) As Double
Private ireg2(1 To 43) As Integer
Private jreg2(1 To 43) As Integer
Private nreg2(1 To 43) As Double
Private ireg3(1 To 40) As Integer
Private jreg3(1 To 40) As Integer
Private nreg3(1 To 40) As Double
Private nreg4(1 To 10) As Double
Private nbound(1 To 5) As Double
Private n0visc(0 To 3) As Double
Private ivisc(1 To 19) As Integer
Private jvisc(1 To 19) As Integer
Private nvisc(1 To 19) As Double
...
'
'
Public Function gammareg1(tau, pi)
'
...
'
'
Private Function gammapireg1(tau, pi)
'
...
'
'
Private Function gammapipireg1(tau, pi)
'
...
'
'
Private Function gammataureg1(tau, pi)
'
...
'
'
Private Function gammatautaureg1(tau, pi)
'
...
'
'
Private Function gammapitaureg1(tau, pi)
'
...
'
'
Private Function gamma0reg2(tau, pi)
'
...
'
'
Private Function gamma0taureg2(tau, pi)
'
...
'
'
Private Function gamma0tautaureg2(tau, pi)
'
...
'
'
Private Function gammarreg2(tau, pi)
'
...
'
'
Private Function gammartaureg2(tau, pi)
'
...
'
'
Private Function gammartautaureg2(tau, pi)
'
...
'
'
Private Function fireg3(tau, delta)
'
' Fundamental equation for region 3
'
    Call InitFieldsreg3
    fireg3 = nreg3(1) * Log(delta)
    For i = 2 To 40
...
'
'
Private Function fideltareg3(tau, delta)
'
...
'
'
Private Function fideltadeltareg3(tau, delta)
'
...
'
'
Private Function fitaureg3(tau, delta)
'
...
'
'
Private Function fitautaureg3(tau, delta)
'
...
'
'
Private Function fideltataureg3(tau, delta)
'
...
'
'
Private Function psivisc(tau, delta)
'
...
'
'
Private Function lambthcon(temperature, pressure, tau, delta)
'
...
'
    If temperature >= 273.15 And temperature <= 623.15 And pressure >= pSatW(temperature) And pressure <= 1000# Then
'  region 1
        taus = 1386# / temperature
        pis = pressure / 165.3
        dpidtau = (647.226 * 165.3 * (gammapitaureg1(taus, pis) * 1386# - gammapireg1(taus, pis) * temperature)) / (221.15 * temperature ^ 2 * gammapipireg1(taus, pis))
        ddeltadpi = -(22115000# * gammapipireg1(taus, pis)) / (317.763 * rgas_water * temperature * gammapireg1(taus, pis) ^ 2)
    ElseIf (temperature >= 273.15 And temperature <= 623.15 And pressure > 0 And pressure <= pSatW(temperature)) Or (temperature >= 623.15 And temperature <= 863.15 And pressure > 0 And pressure <= pBound(temperature)) Or (temperature >= 863.15 And temperature <= 1073.15 And pressure > 0 And pressure <= 1000#) Then
'  region 2
        taus = 540# / temperature
        pis = pressure / 10#
        dpidtau = (647.226 * 10# * ((gamma0pitaureg2(taus, pis) + gammarpitaureg2(taus, pis)) * 540# - (gamma0pireg2(taus, pis) + gammarpireg2(taus, pis)) * temperature)) / (221.15 * temperature ^ 2 * (gamma0pipireg2(taus, pis) + gammarpipireg2(taus, pis)))
        ddeltadpi = -(22115000# * (gamma0pipireg2(taus, pis) + gammarpipireg2(taus, pis))) / (317.763 * rgas_water * temperature * (gamma0pireg2(taus, pis) + gammarpireg2(taus, pis)) ^ 2)
    ElseIf temperature >= 623.15 And temperature <= tBound(pressure) And pressure >= pBound(temperature) And pressure <= 1000# Then
'  region 3
        taus = 647.096 / temperature
        deltas = delta * 317.763 / 322#
        dpidtau = (647.226 * rgas_water * (delta * 317.763) ^ 2 * (fideltareg3(taus, deltas) - (647.096 / temperature) * fideltataureg3(taus, deltas))) / (22115000# * 322#)
        ddeltadpi = (22115000# * 322#) / (317.763 * delta * 317.763 * rgas_water * temperature * (2 * fideltareg3(taus, deltas) + (delta * 317.763 / 322#) * fideltadeltareg3(taus, deltas)))
    Else
'  outside range
        dpidtau = 0
        ddeltadpi = 0
    End If
    lamb2 = 0.0013848 / psivisc(tau, delta) * (tau * delta) ^ (-2) * dpidtau ^ 2 * (delta * ddeltadpi) ^ 0.4678 * delta ^ 0.5 * Exp(-18.66 * (1 / tau - 1) ^ 2 - (delta - 1) ^ 4)
...
'
'
Public Function pSatW(temperature)
'
...
'
'
Private Function pBound(temperature)
'
...
'
    ...
    Else
        ...
        pBound = (nbound(1) + nbound(2) * temperature + nbound(3) * temperature ^ 2) * 10#
    End If
'
End Function
'
'
'
Private Function tBound(pressure)
'
...
'
'
    If pressure < 165.292 Or pressure > 1000# Then
        tBound = -1#
    Else
        Call InitFieldsbound
...
'
'
Private Function volreg1(temperature, pressure)
'
...
'
    ...
    volreg1 = rgas_water * temperature * pi * gammapireg1(tau, pi) / (pressure * 100000#)
'
End Function
'
'
'
Private Function energyreg1(temperature, pressure)
'
...
'
'
Private Function entropyreg1(temperature, pressure)
'
...
'
'
Private Function enthalpyreg1(temperature, pressure)
'
...
'
'
Private Function cvreg1(temperature, pressure)
'
...
'
'
' Private Function spsoundreg1(temperature, pressure)
'
...
'    tau = 540# / temperature
'    pi = 0.1 * pressure / 16.53
'    spsoundreg1 = (rgas_water * temperature * (gammapireg1(tau, pi) ^ 2 / ((gammapireg1(tau, pi) - tau * gammapitaureg1(tau, pi)) ^ 2 / (tau ^ 2 * gammatautaureg1(tau, pi)) - gammapipireg1(tau, pi)))) ^ 0.5
'
' End Function
'
'
'
Private Function volreg2(temperature, pressure)
'
...
'
    tau = 540# / temperature
    pi = 0.1 * pressure
    volreg2 = rgas_water * temperature * pi * (gamma0pireg2(tau, pi) + gammarpireg2(tau, pi)) / (pressure * 100000#)
'
End Function
'
'
'
Private Function energyreg2(temperature, pressure)
'
...
'
'
Private Function entropyreg2(temperature, pressure)
'
...
'
'
Private Function enthalpyreg2(temperature, pressure)
'
...
'
'
Private Function cpreg2(temperature, pressure)
'
...
'
'
Private Function cvreg2(temperature, pressure)
'
...
'
'
' Private Function spsoundreg2(temperature, pressure)
'
...
'    tau = 540# / temperature
'    pi = 0.1 * pressure
'    spsoundreg2 = (rgas_water * temperature * (1 + 2 * pi * gammarpireg2(tau, pi) + pi ^ 2 * gammarpireg2(tau, pi) ^ 2) / ((1 - pi ^ 2 * gammarpipireg2(tau, pi)) + (1 + pi * gammarpireg2(tau, pi) - tau * pi * gammarpitaureg2(tau, pi)) ^ 2 / (tau ^ 2 * (gamma0tautaureg2(tau, pi) + gammartautaureg2(tau, pi))))) ^ 0.5
'
' End Function
'
'
'
Private Function pressreg3(temperature, density)
'
...
'
    ...
    pressreg3 = density * rgas_water * temperature * delta * fideltareg3(tau, delta) / 100000#
'
End Function
'
'
'
Private Function energyreg3(temperature, density)
'
...
'
'
Private Function entropyreg3(temperature, density)
'
...
'
'
Private Function enthalpyreg3(temperature, density)
'
...
'
'
Private Function cpreg3(temperature, density)
'
...
'
'
Private Function cvreg3(temperature, density)
'
...
'
'
' Private Function spsoundreg3(temperature, density)
'
...
'    tau = tc_water / temperature
'    delta = density / dc_water
'    spsoundreg3 = (rgas_water * temperature * (2 * delta * fideltareg3(tau, delta) + delta ^ 2 * fideltadeltareg3(tau, delta) - (delta * fideltareg3(tau, delta) - delta * tau * fideltataureg3(tau, delta)) ^ 2 / (tau ^ 2 * fitautaureg3(tau, delta)))) ^ 0.5
'
' End Function
'
'
...
'
    For j = 1 To 1000
        ...
        densnew = densold + (pressure * 100000# - rgas_water * temperature * densold ^ 2 / dc_water * fideltareg3(tau, delta)) / derivprho
        diffdens = Abs(densnew - densold)
        If diffdens < 5e-06 Then
            densreg3 = densnew
            Exit Function
        End If
        densold = densnew
...
'
'
Public Function densW(temperature, pressure)
'
...
'
'
Public Function energyW(temperature, pressure)
'
...
'
'
Public Function entropyW(temperature, pressure)
'
...
'
'
Public Function enthalpyW(temperature, pressure)
'
...
'
'
Public Function cvW(temperature, pressure)
'
...
'
'
' Public Function spsoundW(temperature, pressure)
'
...
'
'
Public Function viscW(temperature, pressure)
'
...
'
    If temperature >= 273.15 And temperature <= 1073.15 And pressure > 0 And pressure <= 1000# Then
        density = densW(temperature, pressure)
        delta = density / 317.763
        tau = 647.226 / temperature
        viscW = 5.5071e-05 * psivisc(tau, delta)
    Else
'  outside range
        viscW = -1#
    End If
...
'
'
Public Function thconW(temperature, pressure)
'
...
'
'
Public Function densSatLiqTW(temperature)
'
...
'
'
Public Function densSatVapTW(temperature)
'
...
'  region 2
    ...
    ElseIf temperature > 623.15 And temperature <= tc_water Then
'  region 3
        pressure = pSatW(temperature) - 1e-05
...
'
'
Public Function densSatLiqPW(pressure)
'
...
'
'
Public Function densSatVapPW(pressure)
'
...
'
'
Public Function energySatLiqTW(temperature)
'
...
'
'
Public Function energySatVapTW(temperature)
'
...
'
'
Public Function energySatLiqPW(pressure)
'
...
'
'
Public Function energySatVapPW(pressure)
'
...
'
'
Public Function entropySatLiqTW(temperature)
'
...
'
'
Public Function entropySatVapTW(temperature)
'
...
'
'
Public Function entropySatLiqPW(pressure)
'
...
'
'
Public Function enthalpySatLiqTW(temperature)
'
...
'
'
Public Function enthalpySatVapTW(temperature)
'
...
'
'
Public Function enthalpySatLiqPW(pressure)
'
...
'
'
Public Function enthalpySatVapPW(pressure)
'
...
'
'
Public Function cpSatLiqTW(temperature)
'
...
'
'
Public Function cpSatVapTW(temperature)
'
...
'
'
Public Function cpSatLiqPW(pressure)
'
...
'
'
Public Function cpSatVapPW(pressure)
'
...
'
'
Public Function cvSatLiqTW(temperature)
'
...
'
'
Public Function cvSatVapTW(temperature)
'
...
'
'
Public Function cvSatLiqPW(pressure)
'
...
'
'
' Public Function spsoundSatLiqTW(temperature)
'
...
'
'
' Public Function spsoundSatVapTW(temperature)
'
...
'
'
' Public Function spsoundSatLiqPW(pressure)
'
...
'
'
' Public Function spsoundSatVapPW(pressure)
'
...
'
'
Public Function viscSatLiqTW(temperature)
'
...
'  region 1
        ...
        delta = density / 317.763
        tau = 647.226 / temperature
        viscSatLiqTW = 5.5071e-05 * psivisc(tau, delta)
    ElseIf temperature > 623.15 And temperature <= tc_water Then
'  region 3
        pressure = pSatW(temperature)
        density = densreg3(temperature, pressure)
...
'
'
Public Function viscSatVapTW(temperature)
'
...
'  region 2
        ...
        delta = density / 317.763
        tau = 647.226 / temperature
        viscSatVapTW = 5.5071e-05 * psivisc(tau, delta)
    ElseIf temperature > 623.15 And temperature <= tc_water Then
'  region 3
        pressure = pSatW(temperature) - 1e-05
        density = densreg3(temperature, pressure)
...
'
'
Public Function viscSatLiqPW(pressure)
'
...
'  region 1
        ...
        delta = density / 317.763
        tau = 647.226 / temperature
        viscSatLiqPW = 5.5071e-05 * psivisc(tau, delta)
    ElseIf pressure > pSatW(623.15) And pressure <= pc_water Then
'  region 3
        temperature = tSatW(pressure)
        pressure = pressure + 1e-05
...
'
'
Public Function viscSatVapPW(pressure)
'
...
'  region 2
        ...
        delta = density / 317.763
        tau = 647.226 / temperature
        viscSatVapPW = 5.5071e-05 * psivisc(tau, delta)
    ElseIf pressure > pSatW(623.15) And pressure <= pc_water Then
'  region 3
        temperature = tSatW(pressure)
        pressure = pressure - 1e-05
...
'
'
Public Function thconSatLiqTW(temperature)
'
...
'
'
Public Function thconSatVapTW(temperature)
'
...
'
'
Public Function thconSatLiqPW(pressure)
'
